...
def call_result(label_result, n1):  
    ...
    TOTP = get_opt()
    if (code==TOTP):
        subprocess.call(resource_path('hello.exe'), shell=True)  #這裡的 hello.exe 可換掉成要包裝的 exe
    return
...
tk.Button(root,width=10, text="確認與退出", command=lambda: [f() for f in [root.quit, call_result]]).grid(row=3, column=0)
root.mainloop()
